...
## 工具调用结果可视化(2026-04-26)

之前用户问"你刚搜了哪些网页?",agent 只能凭借自己回复里内联的 markdown 链接交差;tool_result 在 SSE 上只有 `{ ok }`,搜出来的标题/摘要全部丢回模型上下文,UI 看不见。这一版把搜索类工具的结果做成 ChatGPT-style 引用卡。

- `services/agent-runtime/src/index.ts`:把 tool 回调的响应解析成 JSON 后,跟着原有 `{ id, ok }` 一起作为 `tool_result.data` 推给客户端;同时透传 `name` 方便客户端不必再用 id 反查上一条 `tool_call` 事件
- `packages/db/src/schema.ts`:`MessageMetadata` 加 `toolInvocations: ToolInvocation[]` 字段(每条带 `name` / `query` / `results | fetched` / `provider` / `error`)。jsonb 列复用,无 schema 迁移
- `apps/web/src/lib/chat/stream.ts`:在 SSE 转发的同时累计 `tool_call` + `tool_result`,过滤白名单(`web_search` / `search_lyrics` / `search_music` / `fetch_url`,记忆类工具不进卡片),把搜索命中(标题/url/摘要)持久化到 `metadata.toolInvocations`。这样刷新页面卡片不丢
- `apps/web/src/components/ChatPanel.tsx`:新增 `<ToolInvocationsCard>` 折叠卡渲染在 agent 气泡的 `<ThinkingPanel>` 之上;SSE 实时分支额外消费 `tool_call` / `tool_result` 事件,工具调用瞬间挂一个 "搜索网页中…" pending 行,`tool_result` 到达后原地解析成命中列表;`/api/messages` 历史加载、`refetchMessages` 重连兜底、`loadOlderMessages` 翻页都补了 `toolInvocations` 反序列化

注意点 / 已知坑:
- 群聊场景下,**只有触发该回复的用户**走的是 SSE 直连流,可以看到 pending → resolved 的过渡;同房间的其他成员目前从 Redis pub/sub 收到的 `agent-message` 事件没带 `toolInvocations`,只在下次刷新历史时才看到卡片。要做真·实时多端同步需要在 `lib/redis.ts` 的 `RoomEvent.message` 加字段并在 realtime-gateway 透传,本版未做
- 卡片只显示 SearchHit 元信息(title / host / 短 snippet),不展开 fetch_url 的完整 8K 字符正文 —— 那个塞进 metadata 会让 jsonb 列吃成肥猪,而且模型已经把要点写进正文了

## TTS 每句重复两遍 真·根因(2026-04-26)

上一次(31799be)以为是 MiniMax 末尾事件 `is_final: true` 重夹一份完整音频,加了 `if (evt.is_final) finalSeen = true` 的过滤。**这个改动从来没生效过** —— 用 curl 直打 `/t2a_v2 stream=true` 抓到的 SSE 里根本不存在 `is_final` 字段,MiniMax 用的是 `data.status`:

  - `data.status: 1` —— 流式块(行 1/3/5/7/9/11)
  - `data.status: 2` —— 终结事件 + **完整音频重夹**(67380 字节,与 `extra_info.audio_size` 完全一致)

所以 `finalSeen` 永远是 false,67KB 的尾部重夹还是被照常 enqueue 进 MSE buffer,跟前面流出来的 66KB 正常块拼起来 → 一句话播两遍。

- `apps/web/src/lib/tts/minimax.ts`:`makeSseAudioTransformer` 改成监听 `evt.data.status === 2`,命中后置 `terminated = true` 并丢弃当前及之后所有 audio。注释里把"docs 撒谎"那段更正成"`is_final` 字段不存在,标志位是 `data.status`,curl 验证过"
- 离线回放验证:把抓到的 14 行原始 SSE 喂回新 transformer,emitted 66285 字节(status:1 总和) / skipped 67380 字节(status:2 那条),比例 1.017 —— 与不修复时的 ~2x 播放时长形成对照

为什么之前 7412ae3 的 `playedCursorsRef` 防御 guard 没救场:它只防"同一段文本被 ship 给 MiniMax 两次",而本 bug 是同一次 MiniMax 调用内部返回的 audio bytes 自带重复,在 transformer 这层就该过滤掉,前端 cursor guard 看不到这层。

## 撤掉「无耳机时静音」开关(2026-04-26)

用户反馈这个开关从来没正常工作过 —— `enumerateDevices()` 在大多数浏览器/设备上拿不到 `audiooutput` 的 label(MediaDevices spec 要求先有麦克风权限才返回非空 label,而我们的语音模式只读不录),命中率低到不如不要。**整套裁掉**比再加二级 fallback 更干净:

- 删 `apps/web/src/lib/audio/headphone-check.ts`
- `apps/web/src/components/ChatPanel.tsx`:移掉 `isHeadphonesConnected` import、`ttsAllowedRef` 的声明 / 重置 / `tryAdvanceTts` 检查、以及 `handleSubmit` 里整段 per-reply 耳机检测块
- `apps/web/src/app/me/page.tsx`:删 🎧「无耳机时静音」整张卡片和对应的 `muteWithoutHeadphones` state / effect / toggler

`localStorage.tts-mute-without-headphones` key 变成孤儿,留着不管 —— 用户清浏览器存储或下次重置时自然消失,加 cleanup hook 反而多事。
