...
export const imageToolDefs = [
  {
    ...
    function: {
      name: "generate_image",
      description:
        "Generate an image from a text prompt (+ optional reference images). ASYNC — returns immediately with { messageId, queued: true }; real image swaps in ~20s.\n\nCALL ONLY when the user EXPLICITLY asks to draw / paint / generate / create an image. Strict triggers:\n  (1) Chinese explicit-draw verbs: 画一张 / 画一个 / 画个 / 画下来 / 画 X 给我 / 帮我画 / 给我画 / 来张图 / 来一张图\n  (2) English explicit-draw verbs: 'draw' / 'paint' / 'generate' / 'create' followed by 'a/an/me/the' + a concrete noun\n  (3) IMAGE-TO-IMAGE / EDIT: 把这张图改成 X / 改成 X 风格 / 加点 X / 这两张融合 — pass source msgId in referenceMessageIds.\n\nDO NOT CALL when the user is:\n  - asking ABOUT an existing image ('这画的是什么 / 你画的是哪种天气 / 颜色不太对 / 这张图怎么是这样')\n  - chatting / asking-about-real-life ('我今晚吃外面 / 今天天气怎样 / 我们聊聊天 / 你觉得呢')\n  - expressing emotion or feedback ('真笨 / 好可爱 / 真好看 / 你不行 / 这张不喜欢')\n  - pushing back / correcting ('别一直画图 / 我没让你画 / 现在怎么一直画图啊')\n  - using 'show me' / '给我看' in a NON-VISUAL sense ('show me how' = explain; '让我看看你的逻辑' = reasoning, not picture)\n  - asking you to describe / explain / discuss something (use words, not images)\n\nWhen in doubt, DON'T call — an unwanted image annoys the user AND wastes ~20s. A text reply is reversible; an image generation isn't.\n\nAfter a successful tool_call, brief acknowledgment ('画着呢～' / '我开始画了') is fine. Writing those WITHOUT having emitted the tool_call this turn is a forbidden hallucination (system rule #10).\n\nREMEMBER the returned messageId — if the user later says '停 / 别画了 / 取消', call cancel_image_generation({messageId}). DO NOT paste the URL in your reply. One generate_image call per request.",
      parameters: {
        type: "object",
        required: ["prompt"],
        properties: {
          ...
          referenceMessageIds: {
            ...
            items: { type: "string" },
            description:
              "Optional msgIds of image messages in this room to use as visual references for image-to-image generation. Read these from the inline '[图片#N (msgId=xxx)]' markers above (you painted some of these yourself — '你画的' marker). 1-4 ids supported; each must be an image message in the SAME room.\n\nPASS THIS when the user is continuing / modifying / re-rendering a subject from a prior image, including:\n  - Direct edit: '把这张改成 X' / '改成 X 风格' / '加点 X' / '用 X 滤镜'\n  - Style transfer: '换成赛博朋克风' / '换成水彩'\n  - Fuse images: '把这两张融合' / '把猫放到那张背景里'\n  - SAME-SUBJECT re-render — different angle / pose / outfit / scene:\n      '画一张全身的' / '来个侧面' / '画他穿西装的' / '同一个人换个发型' / '在第一张图基础上 X' — pass the prior subject's msgId so the model paints the SAME person, not a randomized one.\n  - Numbered references: '第一张那个再来一张' / '上面那张换个表情'\n\nWHEN A USER ASKS TO RE-RENDER, FOLLOW UP, OR ITERATE ON SOMETHING YOU JUST DREW, you almost always need this — without it the model has to recreate the subject from scratch and won't match. If you painted Lee earlier in this conversation and the user says '画他全身的', pass Lee's image msgId here.\n\nOmit only for genuinely fresh images with no relation to anything prior in the room.",
          },
        },
      },
    },
